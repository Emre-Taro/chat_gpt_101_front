    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
          ...
          { sender: 'assistant', text: typeof data.assistant.content === 'string' ? data.assistant.content : JSON.stringify(data.assistant.content) }
      ]);
    } catch(error){
      if(error instanceof Error) {
        setError(error.message);
    ...
    try {
      ...
      if (result.filename) {
        ...
        if (aiData.assistant && aiData.assistant.content) {
          ...
          // Check if the content is a string, if not, convert it
          const assistantText = typeof aiData.assistant.content === 'string' 
            ? aiData.assistant.content 
            : JSON.stringify(aiData.assistant.content);
                ...
                <div className={`flex ${isUser ? 'justify-end' : 'justify-center'}`}>
                  {isUser ? (
                    <div className="flex flex-col items-end gap-2 w-full">
                      {msg.text && (
                        <div className="bg-neutral-700 text-white px-6 py-3 rounded-xl min-w-[5%] max-w-[60%]">
                    ...
                    <div className="flex flex-col items-center gap-2 w-full">
                      {msg.text && (
                        <div className="text-white px-6 py-3 rounded-xl w-full max-w-full">
                          {typeof msg.text === 'object' ? JSON.stringify(msg.text) : msg.text}
                        </div>
                      )}
                      {msg.type === "image" && msg.imageFilename && (
      ...
      >
        <div className="w-[650px] bg-[#2e2e2e] text-white rounded-[24px] px-6 py-5 flex flex-col gap-2">
          ...
          <textarea
            ...
            className="bg-transparent outline-none text-white placeholder-gray-400 text-sm mb-[5px] resize-none overflow-hidden border-none"
            style={{ 
              minHeight: '20px',
              maxHeight: '100px',
              lineHeight: '1.2',
